...
status_t ACodec::configureCodec(
    ...
    int32_t UseSecurePath;
    if (!encoder
            && msg->findInt32("UseSecurePath", &UseSecurePath)
            && UseSecurePath) {
        ALOGD("Force to use secure path.");
        OMX_INDEXTYPE index;
        err = mOMX->getExtensionIndex(
                mNode,
                "OMX.google.android.index.configureForceUseSecurePath",
                &index);
        if (err == OK) {
            ConfigureForceUseSecurePathParams params;
            ...
            if (err == OK) {
                ALOGD("Success to configure force use secure path.");
                mForceUseSecurePath = true;

                mFlags |= kFlagIsSecure;
                mFlags |= kFlagIsGrallocUsageProtected;
                mFlags |= kFlagPushBlankBuffersToNativeWindowOnShutdown;
            } else {
                ALOGD("Configure Forcing to use secure path failed w/ err %d", err);
                mForceUseSecurePath = false;
            }
        } else {
            ALOGD("OMX can not support the feature of forcing to use secure path.");
            mForceUseSecurePath = false;
        }
    }
    ...
    if (video) {
        ...
        if (!msg->findInt32("channel-count", &numChannels)) {
            ...
            if (!msg->findInt32("sample-rate", &sampleRate)) {
                ALOGW("sample-rate has not been parsed !");
                sampleRate = 8000;
            }
            OMX_AUDIO_PCMMODETYPE type;
            if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_G711_ALAW)) {
                type = OMX_AUDIO_PCMModeALaw;
            } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_G711_MLAW)) {
                type = OMX_AUDIO_PCMModeMULaw;
            } else {
                type = OMX_AUDIO_PCMModeLinear;
            }
            err = setupG711Codec(encoder, numChannels, sampleRate, type);
        }
...
status_t ACodec::setupG711Codec(bool encoder, int32_t numChannels,
        int32_t sampleRate, OMX_AUDIO_PCMMODETYPE type) {
    if (!strncasecmp(mComponentName.c_str(), "OMX.google.", 11)) {
        ...
        if (encoder) {
            ALOGW("G711 encoding is not supported.");
            return INVALID_OPERATION;
        }
            ...
            switch ((int)audioDef->eEncoding) {
                case OMX_AUDIO_CodingPCM:
                {
                    ...
                    if (strncasecmp(mComponentName.c_str(), "OMX.google.", 11)
                                && mIsEncoder == false
                                && portIndex == kPortIndexInput) {
                        CHECK_EQ((params.nBitPerSample ^ 8u)
                                && (params.nBitPerSample ^ 16u), 0);

                        CHECK_EQ((int)params.eNumData,
                                 (int)OMX_NumericalDataSigned);

                        CHECK_EQ(((int)params.ePCMMode ^ (int)OMX_AUDIO_PCMModeLinear)
                                && ((int)params.ePCMMode ^ (int)OMX_AUDIO_PCMModeALaw)
                                && ((int)params.ePCMMode ^ (int)OMX_AUDIO_PCMModeMULaw), 0);
                    } else {
                ...
                case OMX_AUDIO_CodingAndroidAC3:
                {
                    ...
                    notify->setString("mime", mMime.c_str());
                    notify->setInt32("channel-count", params.nChannels);
                    notify->setInt32("sample-rate", params.nSampleRate);
                    break;
                }
...
bool ACodec::LoadedState::onConfigureComponent(
...

    {
        ...
        if (mCodec->mForceUseSecurePath) {
            notify->setInt32("ForceUseSecurePath", mCodec->mForceUseSecurePath);
        }
