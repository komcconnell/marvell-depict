...
static const struct option OPTIONS[] = {
  ...
  { "crash_recovery", no_argument, NULL, 'a' },
  { NULL, 0, NULL, 0 },
};
...
prepend_title(const char* const* headers) {
    ...
    *(h++) = "Depict system recovery <" EXPAND(RECOVERY_API_VERSION) "e>";
    *(h++) = recovery_version;
    *(h++) = "";
...
static int
do_crash_recovery(Device* device) {
    int status = INSTALL_SUCCESS;

    const char* MENU_HEADERS[] = { "Use up/down to move highlight;",
                                   "enter button to select Reboot or factory reset",
                                   "",
                                   NULL };

    const char** headers = prepend_title(MENU_HEADERS);

    const char* items[] = { " Reboot device",
...
int system_cmd(const char * cmdstring)
{
    int status;
    pid_t pid;
...
main(int argc, char **argv) {
    ...
    Volume* v = volume_for_path(LAST_LOG_FILE);
    if (v == NULL) {
        fprintf(stderr, "unknown volume for path [%s]\n", LAST_LOG_FILE);
    } else {
        if (strcmp(v->fs_type, "ext4") == 0 || strcmp(v->fs_type, "vfat") == 0) {
            if (wait_for_device(v->blk_device)) {
                fprintf(stderr, "failed to wait device %s\n", v->blk_device);
            }
        }
    }
    ...
    int wipe_data = 0, wipe_cache = 0, show_text = 0, crash_recovery = 0;
    ...
    }  else if (crash_recovery) {
        status = do_crash_recovery(device);
        if (status != INSTALL_SUCCESS) ui->Print("Crash recovery failed.\n");
    } else if (!just_exit) {
        status = INSTALL_NONE;  // No command specified
        ui->SetBackground(RecoveryUI::NO_COMMAND);
        // Show recovery menu for other cases
        ui->ShowText(true);
    }
    ...
    system_cmd("/bin/test_disp_lvds -p off");
#endif
